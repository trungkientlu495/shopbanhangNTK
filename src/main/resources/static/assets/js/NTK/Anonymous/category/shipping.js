function searchbyShipping() {
	...
	!(document.getElementById("unisex").checked || document.getElementById("tlu").checked || document.getElementById("yody").checked)
		...
		$.ajax({
			type: "POST",
			url: "/Anonymous/searchbyShipping",
			data: { search: nameValue, hoatoc: hoatoc, nhanh: nhanh, tietkiem: tietkiem }, // Truyền tham số 'data' như đã định nghĩa trong phương thức 'searchProducts'
...
function searchAllShipping() {
	...
	(!document.getElementById("unisex").checked && !document.getElementById("tlu").checked && !document.getElementById("yody").checked)
		...
		$.ajax({
			type: "POST",
			url: "/Anonymous/searchAllShipping",
